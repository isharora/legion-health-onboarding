...
function continueFlow(){const map={conditions:'insurance',insurance:'needs',needs:'match',match:'patient'};if(state.phase==='conditions'&&!state.conditions.length)return;if(state.phase==='insurance'&&!state.insurance)return;if(state.phase==='needs'&&!state.needs)return;if(state.phase==='needs'){selectRecommendation();go('match');return;}if(state.phase==='match'&&!validSelection(state,state.providerId,state.slotId))return; if(map[state.phase])go(map[state.phase]);}
function back(){if(state.phase==='fit'){state.fit=Math.max(0,state.fit-1);go('fit');}else if(state.phase==='conditions'){state.fit=fitQuestions.length-1;go('fit');}else{const index=phaseOrder.indexOf(state.phase);if(index>0)go(phaseOrder[index-1]);}}

document.addEventListener('click',event=>{
  const btn=event.target.closest('[data-action]');if(!btn)return;
  const {action,value}=btn.dataset;
  if(action==='fit-answer'){const answer=value==='yes';state.answers[state.fit]=answer;state.answers=state.answers.slice(0,state.fit+1);state.eligible=answer===fitQuestions[state.fit].eligible;if(!state.eligible)return go('not-fit');if(state.fit<4){state.fit++;go('fit');}else go('conditions');}
