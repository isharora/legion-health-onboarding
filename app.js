...
function pinOnboardingActions(){
  const selector={conditions:'[data-action="continue"]',insurance:'[data-action="continue"]',needs:'[data-action="continue"]',identity:'#identity-form > .primary',patient:'#save-patient',payment:'.inline-actions:has([data-action="book"])',match:'.selection-confirm'}[state.phase];
  if(!selector)return;
...
function showTask(task){
 if(task==='insurance'&&state.tasks.insurance){openDialog('Your insurance',coverageCard()+`<p>Your coverage details are already on file. We’ll recheck eligibility before your appointment and contact you if anything changes.</p><button class="text-button" data-action="dismiss">Close</button>`);return;}
...
function compactCoverage(){return `<div class="coverage-card compact-coverage"><strong>✓ Coverage found</strong><h2>${state.coverage==='found'?'$25 copay':'Cost estimate pending'}</h2><small>Aetna · ${state.coverage==='found'?'Estimated initial visit':'Coverage confirmed'}</small></div>`;}
function costCard(){const cost=coverageEstimate(state);return `<div class="cost-card compact-coverage"><strong>${state.coverage==='manual'?'Your initial appointment':'✓ Coverage found'}</strong><h2>${state.coverage==='found'?'$25 copay':cost.title}</h2><p class="fine-print">${cost.caveat}</p></div>`;}
...
function previewIntake(channel='web'){
  ...
  clearTimeout(lookupTimer);state.texas=true;state.eligible=true;state.conditions=['Anxiety'];state.needs='existing';state.answers=fitQuestions.map(q=>q.eligible);state.insurance=scenario.verified==='not-found'?'':'Aetna';state.coverage=scenario.verified==='not-found'?'manual':scenario.verified;state.lookupStatus=scenario.verified==='not-found'?'not-found':'found';state.tasks.insurance=scenario.verified!=='not-found';state.locationConfirmed=true;state.booked=true;state.intake.tab=channel;state.intake.previewOnly=channel!=='web';state.intake.unread=channel==='text'?0:1;state.intake.recordStatus=scenario.records;state.intake.demoIndex=0;applyIntakeScenario(state.intake,scenario.records);if(['found','ready'].includes(scenario.records))state.intake.records='connected';selectRecommendation();go('dashboard');
}
