...
const initial = () => ({phase:'conditions',coverage:'manual',locationConfirmed:false,fit:0,answers:[],texas:scenario.texas,eligible:true,conditions:[],insurance:'',needs:'',providerId:'',slotId:'',patient:{first:'Ishita',last:'Testing',email:'ish.g.arora@gmail.com',phone:'(408) 440-6539',dob:'1992-12-21',referral:'Google, Bing, or other search engine'},checks:{terms:true,treatment:true,recording:true},tasks:{},booked:false});
let state = initial();
...
function progress() {
  const sequence=['conditions','needs',...(!scenario.texas?['location-check']:[]),'fit-1','fit-2','fit-4','identity',...(state.coverage==='manual'?['fit-0','insurance']:['coverage'])];
...
function render() {
  ...
  if(state.phase==='identity'){
    ...
    html=progress()+heading('We found your insurance','Confirm your coverage so we can show you providers who accept your plan.')+coverageCard()+`<p class="fine-print">${fitQuestions[0].body}</p><button class="primary" data-action="confirm-coverage">Yes, this is my coverage</button><button class="text-button" data-action="manual-coverage">This isn’t correct / I have other coverage</button><button class="back" data-action="edit-identity">← Edit my details</button>`;
...
function manualInsurance(){state.coverage='manual';state.insurance='';state.tasks.insurance=false;state.answers[0]=undefined;state.fit=0;go('fit');}
...
function back(){
 ...
 if(state.phase==='match')return go(state.coverage==='manual'?'insurance':'coverage');
...
document.addEventListener('click',event=>{
  ...
  if(action==='confirm-coverage'){state.insurance='Aetna';state.tasks.insurance=true;state.answers[0]=false;go('match');}
...
document.addEventListener('submit',e=>{if(e.target.id==='identity-form'){e.preventDefault();submitIdentity(true);return;}if(e.target.id==='patient-form'){e.preventDefault();const fd=new FormData(e.target);state.patient={...state.patient,...Object.fromEntries(['email','phone','referral'].map(k=>[k,fd.get(k)]))};state.checks=Object.fromEntries(['terms','treatment','recording'].map(k=>[k,fd.has(k)]));state.locationConfirmed=fd.has('visit-location');if(state.locationConfirmed)go('payment');}if(e.target.id==='insurance-form'){e.preventDefault();openDialog('Insurance verification','<p>This demo does not connect to an insurer. No coverage has been verified.</p><button class="primary modal-footer" data-action="dismiss">Back to care summary</button>');}});
...
document.addEventListener('click',e=>{if(e.target.id==='confirm-restart'){state=initial();dialog.close();go(state.phase);}});
...
function submitIdentity(lookup){if(!canLookupInsurance(state))return;const form=document.getElementById('identity-form');const consent=form.elements['lookup-consent'];consent.required=lookup;if(!form.reportValidity()){consent.required=true;return;}const fd=new FormData(form);for(const key of ['first','last','dob'])state.patient[key]=fd.get(key).trim();if(!isAdult(state.patient.dob)){state.fit=3;state.eligible=false;go('not-fit');return;}state.eligible=true;state.coverage=lookup?scenario.verified:'manual';if(state.coverage==='not-found')state.coverage='manual';state.insurance='';state.tasks.insurance=false;state.answers[3]=true;state.providerId='';state.slotId='';if(state.coverage==='manual'){manualInsurance();if(lookup)flow.querySelector('.step-enter').insertAdjacentHTML('afterbegin','<div class="notice" role="status">We couldn’t find your insurance. This does not mean you’re uninsured. You can continue by entering it yourself.</div>');}else go('coverage');}
